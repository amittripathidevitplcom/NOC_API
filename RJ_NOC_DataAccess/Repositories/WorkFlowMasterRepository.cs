...
namespace RJ_NOC_DataAccess.Repository
{
    public class WorkFlowMasterRepository : IWorkFlowMasterRepository
    {
        ...
        public bool SaveData(WorkFlowMasterDataModel request)
        {
            ...
            string SqlQuery = " exec USP_SaveWorkFlowMaster @WorkFlowMasterID='" + request.WorkFlowMasterID + "',@DepartmentID='" + request.DepartmentID + "',@SchemeID='" + request.SchemeID + "',";
            SqlQuery += "@ModuleID='" + request.ModuleID + "',@SubModuleID = '" + request.SubModuleID + "',@RoleLevelID = '" + request.RoleLevelID + "',@RoleID = '" + request.RoleID + "',@IPAddress = '" + IPAddress + "',@WorkFlowMasterDetail_Str='" + WorkFlowMasterDetail_Str + "'";
        ...
        {
            string SqlQuery = " USP_IfExistsWorkFlowMaster @SubModuleID='" + SubModuleID + "',@RoleID='" + RoleID + "',@WorkFlowMasterID='" + WorkFlowMasterID + "' ";
        ...
        public List<WorkFlowMasterDataModel> GetWorkFlowMasterList(int WorkFlowMasterID)
        {
            ...
            else
            {
                if (dataSet.Tables[0].Rows.Count > 0)
                {
                    ...
                    dataModels.SchemeID = Convert.ToInt32(dataSet.Tables[0].Rows[0]["SchemeID"]);
                    dataModels.SchemeName = dataSet.Tables[0].Rows[0]["SchemeName"].ToString();
                    dataModels.ModuleID = Convert.ToInt32(dataSet.Tables[0].Rows[0]["ModuleID"]);
                    dataModels.ModuleName = dataSet.Tables[0].Rows[0]["ModuleName"].ToString();
                    dataModels.SubModuleID = Convert.ToInt32(dataSet.Tables[0].Rows[0]["SubModuleID"]);
