        ...
        public async Task<OperationResult<CAAadharResponseForESPRedirect>> CA_eSignPDF(CAGetSignedXmlApiRequest request)
        {
            ...
            try
            {
                ...
                if (reqStatus == true)
                {
                    ...
                    request.RequestJson = "{\"pdfFile1\":\"" + request.pdfBase64 + "\",\"signatureOnPageNumber\":\"" + dt_RSDDATA.Rows[0]["signatureOnPageNumber"].ToString() + "\",\"clientCode\":\"" + dt_esignCredentials.Rows[0]["CAClientCode"].ToString() + "\",\"designation\": \"" + dt_RSDDATA.Rows[0]["designation"].ToString() + "\",\"txn\": \"" + dt_RSDDATA.Rows[0]["TransactionID"].ToString() + "\",\"responseUrl\":\"" + dt_RSDDATA.Rows[0]["successFailureurl"].ToString() + "\" ,\"location\":\"" + dt_RSDDATA.Rows[0]["Location"].ToString() + "\",\"xcord\":\"" + dt_RSDDATA.Rows[0]["xcord"].ToString() + "\",\"ycord\":\"" + dt_RSDDATA.Rows[0]["ycord"].ToString() + "\",\"sigsize\":\"" + dt_RSDDATA.Rows[0]["sigsize"].ToString() + "\"}";
