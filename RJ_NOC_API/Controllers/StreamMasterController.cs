...
namespace RJ_NOC_API.Controllers
{
    ...
    public class StreamMasterController : RJ_NOC_ControllerBase
    {
        ...
        public async Task<OperationResult<List<CommonDataModel_DataTable>>> GetAllStreamList(int DepartmentID)
        {
            CommonDataAccessHelper.Insert_TrnUserLog(0, "GetAllData", 0, "StreamMaster");
            var result = new OperationResult<List<CommonDataModel_DataTable>>();
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.StreamMasterUtility.GetAllStreamList(DepartmentID));
        ...
        public async Task<OperationResult<bool>> SaveData(StreamMasterDataModel request)
        {
            ...
            {
                bool IfExits = false;
                IfExits = UtilityHelper.StreamMasterUtility.IfExists(request.StreamMappingID, request.CourseLevelID, request.CourseID, request.DepartmentID,request.StreamID);
                if (IfExits == false)
                {
