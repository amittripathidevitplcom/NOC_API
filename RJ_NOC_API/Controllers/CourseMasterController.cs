...
namespace RJ_NOC_API.Controllers
{
    ...
    public class CourseMasterController : RJ_NOC_ControllerBase
    {
        ...
        public async Task<OperationResult<List<CommonDataModel_DataTable>>> GetAllCourse(int UserID, string LoginSSOID,int CollegeID,int ApplyNOCID)
        {
            ...
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.CourseMasterUtility.GetAllCourse(LoginSSOID, CollegeID,ApplyNOCID));
        ...
        [HttpGet("GetAllCourseDTE/{CollegeWiseCourseID}/{LoginSSOID}/{UserID}/{collegeID}/{ApplyNOCID}")]
        public async Task<OperationResult<List<CommonDataModel_DataTable>>> GetAllCourseDTE( int CollegeWiseCourseID, string LoginSSOID, int UserID,int collegeID,int ApplyNOCID)
        {
            CommonDataAccessHelper.Insert_TrnUserLog(UserID, "GetAllData", 0, "GetAllCourseDTE");
            var result = new OperationResult<List<CommonDataModel_DataTable>>();
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.CourseMasterUtility.GetAllCourseDTE(LoginSSOID, CollegeWiseCourseID, collegeID, ApplyNOCID));
                result.State = OperationState.Success;
                if (result.Data.Count > 0)
                {
                    result.State = OperationState.Success;
        ...
        public async Task<OperationResult<List<CommonDataModel_DataTable>>> CoursesReport(CourseReportSearchFilter request)
        {
            var result = new OperationResult<List<CommonDataModel_DataTable>>();
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.CourseMasterUtility.CoursesReport(request));
                result.State = OperationState.Success;
                if (result.Data.Count > 0)
                {
                    result.State = OperationState.Success;
            ...
            {
                CommonDataAccessHelper.Insert_ErrorLog("CourseMasterController.CoursesReport", ex.ToString());
                result.State = OperationState.Error;
                result.ErrorMessage = ex.Message.ToString();
            }
            finally
