...
namespace RJ_NOC_API.Controllers
{
    ...
    public class ApplyNocParameterMasterController : RJ_NOC_ControllerBase
    {
        ...
        public async Task<OperationResult<List<ApplyNOCCourseListDataModal>>> GetCourseSubjectByApplyNOCID(int ApplyNOCID, int ParameterID)
        {
            ...
            {
                CommonDataAccessHelper.Insert_ErrorLog("ApplyNocParameterController.GetCourseSubjectByApplyNOCID", ex.ToString());
                result.State = OperationState.Error;
                result.ErrorMessage = ex.Message.ToString();
            }
            finally
        ...
        [HttpGet("GetApplyBTERPaymentHistoryApplicationID/{DTEAffiliationID}/{PaymentFor}")]
        public async Task<OperationResult<List<CommonDataModel_DataTable>>> GetApplyBTERPaymentHistoryApplicationID(int DTEAffiliationID, string PaymentFor)
        {
            var result = new OperationResult<List<CommonDataModel_DataTable>>();
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.ApplyNocParameterMasterUtility.GetApplyBTERPaymentHistoryApplicationID(DTEAffiliationID, PaymentFor));
                result.State = OperationState.Success;
                if (result.Data != null)
                {
                    result.State = OperationState.Success;
