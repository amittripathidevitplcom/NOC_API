...
namespace RJ_NOC_API.Controllers
{
    ...
    public class AgricultureDocumentScrutinyController : RJ_NOC_ControllerBase
    {
        ...
        public async Task<OperationResult<List<CommonDataModel_DataTable>>> UpdateNOCPDFData(int ApplyNocID, int DepartmentID, int CollegeID, string ActionType, int UserId, int RoleID)
        {
            ...
            try
            {
                if (await Task.Run(() => UtilityHelper.AnimalDocumentScrutinyUtility.SaveNOCIssueData(ApplyNocID, DepartmentID, CollegeID, ActionType)))
                {
                    ...
                    {
                        if (await Task.Run(() => UtilityHelper.AgricultureDocumentScrutinyUtility.FinalSavePDFPathandNOC(Path, ApplyNocID, DepartmentID, RoleID, UserId, "", "UpdateGeneratePDF")))
                        {
                            result.State = OperationState.Success;
        ...
        public async Task<OperationResult<List<CommonDataModel_DataTable>>> FinalNOCRejectRelese(GenerateNOC_DataModel request)
        {
            ...
            try
            {
                bool success = await Task.Run(() => UtilityHelper.AgricultureDocumentScrutinyUtility.FinalSavePDFPathandNOC("", request.ApplyNOCID, request.DepartmentID, 0, request.UserID, request.NOCIssuedRemark, request.Status));
                if (success)
                {
        ...
        private string GeneratePDF(int ApplyNOCID)
        {
            ...
            List<CommonDataModel_DataTable> dt = UtilityHelper.AgricultureDocumentScrutinyUtility.GetGeneratePDFData(ApplyNOCID, 0, 0, "GetDataForPDF");
            ...
            string mimetype = "";
            int extension = 1;
            var path = (System.IO.Path.Combine(Directory.GetCurrentDirectory(), "Reports")) + "\\AgriNOC_Print.rdlc";
            ...
            localReport.AddDataSource("AgriNOCPDF", dt[0].data);
        ...
        //        sb.Append("<tr><td height='30px;'></td></tr>");
        //        sb.Append("<tr><td><p>NOC has been issued to " + CollegeName + " College of Agriculture Department. I have no objection to this, I am marking it with my seal.</p></td></tr>");
        //        sb.Append("<tr><td height='30px;'></td></tr>");
        //        sb.Append("<tr><td height='30px;'></td></tr>");
        //        sb.Append("<tr><td height='30px;'></td></tr>");
