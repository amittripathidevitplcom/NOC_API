...
namespace RJ_NOC_API.Controllers
{
    ...
    public class SocietyMasterController : RJ_NOC_ControllerBase
    {
        ...
        [HttpGet("Check30Female/{CollegeID}")]
        public async Task<OperationResult<List<CommonDataModel_DataTable>>> Check30Female(int CollegeID)
        {
            var result = new OperationResult<List<CommonDataModel_DataTable>>();
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.SocietyMasterUtility.Check30Female(CollegeID));
                result.State = OperationState.Success;
                if (result.Data.Count > 0)
                {
                    result.State = OperationState.Success;
            ...
            {
                CommonDataAccessHelper.Insert_ErrorLog("SocietyMasterController.Check30Female", ex.ToString());
                result.State = OperationState.Error;
                result.ErrorMessage = ex.Message.ToString();
            }
            finally
