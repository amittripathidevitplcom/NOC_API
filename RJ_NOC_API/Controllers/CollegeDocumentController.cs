...
namespace RJ_NOC_API.Controllers
{
    ...
    public class CollegeDocumentController : RJ_NOC_ControllerBase
    {
        ...
        [HttpGet("GetOtherDocumentByID/{OtherDocumentID}")]
        public async Task<OperationResult<List<CommonDataModel_DataTable>>> GetOtherDocumentByID(int OtherDocumentID)
        {
            CommonDataAccessHelper.Insert_TrnUserLog(OtherDocumentID, "GetOtherDocumentByID", 0, "CollegeDocument");
            var result = new OperationResult<List<CommonDataModel_DataTable>>();
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.CollegeDocumentUtility.GetOtherDocumentByID(OtherDocumentID));
                result.State = OperationState.Success;
                if (result.Data.Count > 0)
                {
                    result.State = OperationState.Success;
            ...
            {
                CommonDataAccessHelper.Insert_ErrorLog("CollegeDocumentController.GetOtherDocumentByID", ex.ToString());
                result.State = OperationState.Error;
                result.ErrorMessage = ex.Message.ToString();
            }
            finally
