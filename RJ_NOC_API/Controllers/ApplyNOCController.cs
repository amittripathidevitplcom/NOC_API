...
namespace RJ_NOC_API.Controllers
{
    ...
    public class ApplyNOCController : RJ_NOC_ControllerBase
    {
        ...
        [HttpPost("SaveBTERDocumentScrutiny")]
        public async Task<OperationResult<bool>> SaveBTERDocumentScrutiny(BTERDocumentScrutinyDataModel request)
        {
            var result = new OperationResult<bool>();
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.ApplyNOCUtility.SaveBTERDocumentScrutiny(request));
                if (result.Data)
                {
                    CommonDataAccessHelper.Insert_TrnUserLog(request.ApplyAffiliationID, "SaveBTERDocumentScrutiny", 0, "ApplyBTER");
                    result.State = OperationState.Success;
                    result.SuccessMessage = "save successfully .!";
                }
                else
            ...
            {
                CommonDataAccessHelper.Insert_ErrorLog("ApplyNOCController.SaveBTERDocumentScrutiny", e.ToString());
                result.State = OperationState.Error;
                result.ErrorMessage = e.Message.ToString();
            }
            finally
        ...
        [HttpGet("GetAppliedParameterEssentialityForAffiliationorder")]
        public async Task<OperationResult<List<CommonDataModel_DataTable>>> GetAppliedParameterEssentialityForAffiliationorder()
        {
            //CommonDataAccessHelper.Insert_TrnUserLog(UserID, "GetNOCIssuedReportListForAdmin", 0, "ApplyNOCController");
            var result = new OperationResult<List<CommonDataModel_DataTable>>();
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.ApplyNOCUtility.GetAppliedParameterEssentialityForAffiliationorder());
                result.State = OperationState.Success;
                if (result.Data.Count > 0)
                {
                    result.State = OperationState.Success;
            ...
            {
                CommonDataAccessHelper.Insert_ErrorLog("ApplyNOCController.GetAppliedParameterEssentialityForAffiliationorder", ex.ToString());
                result.State = OperationState.Error;
                result.ErrorMessage = ex.Message.ToString();
            }
            finally
