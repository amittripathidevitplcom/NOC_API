...
namespace RJ_NOC_API.Controllers
{
    ...
    public class OldNOCDetailController : RJ_NOC_ControllerBase
    {
        ...
        [HttpGet("GetOldNOCDetailList_DepartmentCollegeWise/{DepartmentID}/{CollegeID}/{OldNocID}/{ApplyNOCID}")]
        public async Task<OperationResult<List<OldNocDetailsDataModel>>> GetOldNOCDetailList_DepartmentCollegeWise(int DepartmentID, int CollegeID, int OldNocID,int ApplyNOCID)
        {
            var result = new OperationResult<List<OldNocDetailsDataModel>>();
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.OldNOCDetailUtility.GetOldNOCDetailList_DepartmentCollegeWise(DepartmentID, CollegeID, OldNocID, ApplyNOCID));
                result.State = OperationState.Success;
        ...
        [HttpGet("GetOldNOCDetailListForPDF/{DepartmentID}/{CollegeID}")]
        public async Task<OperationResult<List<CommonDataModel_DataSet>>> GetOldNOCDetailListForPDF(int DepartmentID, int CollegeID)
        {
            CommonDataAccessHelper.Insert_TrnUserLog(0, "GetOldNOCDetailListForPDF", 0, "StaffDetail");
            var result = new OperationResult<List<CommonDataModel_DataSet>>();
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.OldNOCDetailUtility.GetOldNOCDetailListForPDF(DepartmentID, CollegeID));
                result.State = OperationState.Success;
                if (result.Data.Count > 0)
                {
                    result.State = OperationState.Success;
