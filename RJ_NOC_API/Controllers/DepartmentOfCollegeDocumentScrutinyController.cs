...
namespace RJ_NOC_API.Controllers
{
    ...
    public class DepartmentOfCollegeDocumentScrutinyController : RJ_NOC_ControllerBase
    {
        ...
        public async Task<OperationResult<List<CommonDataModel_DataTable>>> GetRevertedTabData(int ApplyNOCID,int CollegeID)
        {
            ...
            {
                CommonDataAccessHelper.Insert_ErrorLog("DepartmentOfCollegeDocumentScrutiny.GetRevertedTabData", ex.ToString());
                result.State = OperationState.Error;
                result.ErrorMessage = ex.Message.ToString();
            }
            finally
        ...
        [HttpPost("GetClassWiseStaticReport")]
        public async Task<OperationResult<List<CommonDataModel_DataTable>>> GetClassWiseStaticReport(SearchFilterDataModel request)
        {
            var result = new OperationResult<List<CommonDataModel_DataTable>>();
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.DepartmentOfCollegeScrutinyUtility.GetClassWiseStaticReport(request));
                result.State = OperationState.Success;
                if (result.Data != null)
                {
                    result.State = OperationState.Success;
            ...
            {
                CommonDataAccessHelper.Insert_ErrorLog("DepartmentOfCollegeDocumentScrutiny.GetClassWiseStaticReport", ex.ToString());
                result.State = OperationState.Error;
                result.ErrorMessage = ex.Message.ToString();
            }
            finally
        ...
        [HttpPost("GetSubjectWiseStaticReport")]
        public async Task<OperationResult<List<CommonDataModel_DataTable>>> GetSubjectWiseStaticReport(SearchFilterDataModel request)
        {
            var result = new OperationResult<List<CommonDataModel_DataTable>>();
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.DepartmentOfCollegeScrutinyUtility.GetSubjectWiseStaticReport(request));
                result.State = OperationState.Success;
                if (result.Data != null)
                {
                    result.State = OperationState.Success;
        ...
        public async Task<OperationResult<bool>> OpenApplication(OpenApplicationDataModel request)
        {
            var result = new OperationResult<bool>();
            try
            {
                result.Data = await Task.Run(() => UtilityHelper.DepartmentOfCollegeScrutinyUtility.OpenApplication(request));
                if (result.Data)
                {
                    result.State = OperationState.Success;
            ...
            {
                CommonDataAccessHelper.Insert_ErrorLog("DepartmentOfCollegeDocumentScrutiny.OpenApplication", e.ToString());
                result.State = OperationState.Error;
                result.ErrorMessage = e.Message.ToString();
            }
            finally
